...
Implemented the requested P1 controls without changing PostgreSQL data or Render secrets. `/auth/login` now applies a bounded in-memory limiter of five failed attempts per normalized IP+username within 15 minutes, returns HTTP 429 with `Retry-After` after the limit, and prunes expired/overflow buckets. The limiter is intentionally process-local; a future multi-instance deployment should move counters to shared storage.
